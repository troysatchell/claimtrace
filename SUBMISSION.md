...
|---|---|---|
| Public model checkpoint on Hugging Face Hub + exact commit hash | `publish.py` writes `results/publish.json` (repo ids + revisions) | ⛔ `hf auth login`, then `python3 publish.py --run q270 --user <hf-user>` |
...
| Pinned versions | eval-code commit: `<dir>/run.json → eval_code_commit`; training commit + adapter sha256: `results/train/<run>/summary.json`; HF revision: `results/publish.json` | ⛔ HF revision pending publish |
...
| Deliverable | Where | Status |
|---|---|---|
| Dataset published | `publish.py` (dataset repo) | ⛔ needs HF login |
| Model on HF Hub + running inference demo | `publish.py`; demo: `compare.py` (local) | ⛔ |
